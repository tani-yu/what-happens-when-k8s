...
Cgroupsは、Linuxがリソースを割り与える方法であるため（リソース使用量を監視する警察のような）重要になります。Dockerはこれら両方のKernelの機能を使い、リソースが確保され隔離が強制されたプロセス（つまりコンテナ）をホストします。詳細は `b0rk` の驚くべき投稿を確認して下さい。 [What even is a Container?](https://jvns.ca/blog/2016/10/10/what-even-is-a-container/)
